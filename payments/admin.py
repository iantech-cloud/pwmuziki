...
class TransactionInline(admin.TabularInline):
    model = Transaction
    extra = 0
    readonly_fields = ('provider', 'provider_reference', 'amount', 'status', 'raw_response', 'created_at')
    can_delete = False


@admin.register(Invoice)
class InvoiceAdmin(admin.ModelAdmin):
    list_display = ('number', 'booking', 'client', 'photographer', 'total_amount', 'is_paid', 'due_date', 'issued_at')
    list_filter = ('is_paid', 'due_date', 'issued_at')
...
class TransactionAdmin(admin.ModelAdmin):
    list_display = ('id', 'invoice', 'payer', 'provider', 'amount', 'status', 'provider_reference', 'created_at')
    ...
    fieldsets = (
        ('Payment', {'fields': ('invoice', 'payer', 'provider', 'amount', 'status')}),
